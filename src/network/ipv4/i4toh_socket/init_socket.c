...
/*   Updated: 2024/05/27 11:24:22 by brda-sil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_i4toh_init_socket(void)
{
	...
	if (sock == -1)
	{
		ft_perr("ft_i4toh_init_socket: Permission denied\n");
		return (sock);
	}
	...
	{
		ft_perr("ft_i4toh_init_socket: SO_RCVTIMEO\n");
		return (sock);
	}
	if (setsockopt(sock, IPPROTO_IP, IP_HDRINCL, (int [1]){1}, \
															sizeof(int)) == -1)
	{
		ft_perr("ft_i4toh_init_socket: IP_HDRINCL\n");
		return (sock);
	}
	return (sock);
}
