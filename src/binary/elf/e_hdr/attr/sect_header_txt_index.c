...
t_eh_shstrndx	ft_get_e_hdr_shstrndx(t_elf_ehdr *e_hdr)
{
	...
	if (NM_CLASS == ELFCLASS64)
		shstrndx = e_hdr->_64->e_shstrndx;
	else if (NM_CLASS == ELFCLASS32)
		shstrndx = e_hdr->_32->e_shstrndx;
	return (shstrndx);
}
...
t_elf_error	ft_check_e_hdr_shstrndx(t_elf_ehdr *e_hdr)
{
	...
	shstrndx = ft_get_e_hdr_shstrndx(e_hdr);
	fmt = ELF_STR_EHDR_SECT_TXT_INDEX SEP ELF_FMT_NB "\n";
	ft_pdeb(fmt, shstrndx);
	return (SUCCESS);
}
